...
namespace Quartz
{
	WORD sWin32DefaultLogColors = 0;
	FILE* spWin32OutputStream = 0;
	...
	void Log::InitLogging()
	{
		...
		sWin32DefaultLogColors = consoleInfo.wAttributes;
		spWin32OutputStream = pStream;
		...
		const char* severityName, const char* format, ...)
	{
		...
		WORD forgroundColor = (foreground == LOG_COLOR_DEFAULT) ? sWin32DefaultLogColors & 0x0F : sWin32LogColors[foreground];
		WORD backgroundColor = (background == LOG_COLOR_DEFAULT) ? sWin32DefaultLogColors & 0xF0 : (sWin32LogColors[background] << 4) & 0xF0;
		HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
		...
		fwprintf(spWin32OutputStream, pWideBuffer);

		va_end(args);

		SetConsoleTextAttribute(hConsole, sWin32DefaultLogColors);
	...
	void Log::_Log(LogLevel level, LogColor foreground, LogColor background, const char* severityName, const wchar_t* format, ...)
	{
		...
		vfwprintf(spWin32OutputStream, prefixedFormat, args);
	...
	void Log::_LogRaw(LogColor foreground, LogColor background, const wchar_t* format, ...)
	{
		...
		vfwprintf(spWin32OutputStream, format, args);
